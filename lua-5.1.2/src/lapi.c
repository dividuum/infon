...
LUA_API lua_CFunction lua_atcpu_exceeded(lua_State *L, lua_CFunction cpuf) {
  lua_CFunction old;
  lua_lock(L);
  old = G(L)->cpu_exceeded;
  G(L)->cpu_exceeded = cpuf;
  lua_unlock(L);
  return old;
}
...
LUA_API void lua_set_cycles(lua_State *L, int cycles) {
    lua_lock(L);
    G(L)->cycles = cycles;
    lua_unlock(L);
}

LUA_API int lua_get_cycles(lua_State *L) {
    int cycles;
    lua_lock(L);
    cycles = G(L)->cycles;
    lua_unlock(L);
...
LUA_API int lua_gc (lua_State *L, int what, int data) {
  ...
  switch (what) {
    ...
    case LUA_GCESTIMATED: {
      res = cast_int(g->estimate >> 10);
      break;
    }
